...
Note: username/like tags are not processed by this script (as they are in
legacy_tags_to_comments.py).  That's because there's a new script to turn
all username/like into #like comments. See legacy_like_tags_to_comments.py
"""
...
def extractCommentsImagesAndURLs(config):
    comments = []
    usernames = set()
    for tagName in (u'comment', u'image', u'url'):
        result = config.store.find((TagValue, User, AboutTagValue),
                                   TagValue.tagID == Tag.id,
                                   TagValue.creatorID == User.id,
                                   User.id != config.fluidinfoUser.id,
...
if __name__ == '__main__':
    ...
    # USERNAME/{COMMENT/IMAGE/URL} TAGS
    start = time.time()
    previousCount = len(comments)
    result = extractCommentsImagesAndURLs(config)
    usernames.update(result['usernames'])
    comments.extend(result['comments'])
    elapsed = time.time() - start
    print 'Extracted %d comment/image/url comments in %s seconds' % (
        len(comments) - previousCount, elapsed)
